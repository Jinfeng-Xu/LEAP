        ...
        if self.dataset == 'zinc_standard_agent':
            ...
            for i in range(len(smiles_list)):
                ...
                #Chem.SanitizeMol(rdkit_mol,
                                 #sanitizeOps=Chem.SanitizeFlags.SANITIZE_KEKULIZE)
                data = mol_to_graph_data_obj_simple(rdkit_mol)
                max_node_num = data.x.shape[0] if data.x.shape[0] > max_node_num else max_node_num
                # manually add mol id
                data.id = torch.tensor(
                    [i])  # id here is the index of the mol in
                # the dataset
                ...
                data_list.append(data)
                data_smiles_list.append(smiles_list[i])
            print('max_node_num', max_node_num)  # 132
...
# NB: only properly tested when dataset_1 is chembl_with_labels and dataset_2 is pcba_pretrain
